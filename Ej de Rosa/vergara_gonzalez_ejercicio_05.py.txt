...
#primero reemplazo los valores no alfanuméritos por espacios como he hecho en ejercicios anteriores
...
for n in range(len(lista)):
    try:
        if float(lista[n])%1!=0:  #filtra si un número es entero o tiene decimales(con lo cual su resto al dividir de forma entera por 1 es distinto de 0)
            lista10.append(float(lista[n]))
    except ValueError:
        print("Valor no numérico, no convertido")
        continue
